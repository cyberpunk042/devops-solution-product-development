...
# Must happen BEFORE API token creation — Plane rejects API calls until
# the instance is marked as set up and verified.
...
# Clear instance cache so API picks up new config without a container restart.
# Plane caches instance state in Django's cache backend (Redis/Valkey).
# A container restart would work too, but takes ~40s and causes 502s for other scripts.
log "Flushing API cache..."
CACHE_RESULT=$(docker exec "$API_CONTAINER" python manage.py shell -c "
...
if echo "$CACHE_RESULT" | grep -q "ok"; then
    ...
    log "WARN: Cache flush failed ($CACHE_RESULT) — restarting API container"
    docker restart "$API_CONTAINER" >/dev/null 2>&1
    log "Waiting for API backend..."
    for i in $(seq 1 30); do
        HTTP=$(curl -sS -o /dev/null -w "%{http_code}" --max-time 5 \
            "$PLANE_URL/api/v1/users/me/" 2>/dev/null || echo "000")
        if [ "$HTTP" = "401" ] || [ "$HTTP" = "403" ] || [ "$HTTP" = "200" ]; then
            log "API backend ready (HTTP $HTTP)"
            break
        fi
        sleep 3
    done
...
# Ensure admin role even if member already existed with a lower role
if not wm_created and wm.role < 20:
    wm.role = 20
...
# API Token — delete and recreate to ensure we get the plaintext value
APIToken.objects.filter(label='fleet-devops-ws', user=user, workspace=ws).delete()
...
result = {
    ...
    'api_token': str(token.token),
}
# Print JSON on a single line, flush to avoid buffering
print('PLANE_JSON:' + json.dumps(result), flush=True)
" 2>&1)

# Extract just the JSON line (filter out any Django warnings/banners)
JSON_LINE=$(echo "$OUTPUT" | grep '^PLANE_JSON:' | sed 's/^PLANE_JSON://')
[ -n "$JSON_LINE" ] || die "Django shell did not produce expected output. Raw output: $OUTPUT"

WORKSPACE_SLUG=$(echo "$JSON_LINE" | python3 -c "import json,sys; d=json.load(sys.stdin); print(d['workspace_slug'])")
WORKSPACE_ID=$(echo "$JSON_LINE" | python3 -c "import json,sys; d=json.load(sys.stdin); print(d['workspace_id'])")
PROJECT_ID=$(echo "$JSON_LINE" | python3 -c "import json,sys; d=json.load(sys.stdin); print(d.get('project_id',''))" 2>/dev/null || echo "")
PROJECT_IDENTIFIER=$(echo "$JSON_LINE" | python3 -c "import json,sys; d=json.load(sys.stdin); print(d.get('project_identifier',''))" 2>/dev/null || echo "")
API_TOKEN=$(echo "$JSON_LINE" | python3 -c "import json,sys; d=json.load(sys.stdin); print(d['api_token'])")

# ── Verify API access ──────────────────────────────────────────────────────────

log "Verifying API access (token: ${API_TOKEN:0:24}...)..."
VERIFIED=false
for attempt in 1 2 3 4 5; do
    MEMBERS=$(curl -sS -w "\nHTTP_STATUS:%{http_code}" -H "X-Api-Key: $API_TOKEN" \
        "$PLANE_URL/api/v1/workspaces/$WORKSPACE_SLUG/members/" 2>/dev/null)
    HTTP_STATUS=$(echo "$MEMBERS" | grep "^HTTP_STATUS:" | cut -d: -f2)
    BODY=$(echo "$MEMBERS" | grep -v "^HTTP_STATUS:")
    MEMBER_COUNT=$(echo "$BODY" | python3 -c "import json,sys; d=json.load(sys.stdin); print(len(d) if isinstance(d, list) else len(d.get('results', [])))" 2>/dev/null || echo "0")
    if [ "$MEMBER_COUNT" -gt "0" ] 2>/dev/null; then
        VERIFIED=true
        break
    fi
    log "Attempt $attempt: HTTP $HTTP_STATUS, members=$MEMBER_COUNT — retrying in 5s..."
    log "  Response: ${BODY:0:200}"
    sleep 5
done
[ "$VERIFIED" = "true" ] || die "API access verification failed after 5 attempts (HTTP $HTTP_STATUS, response: ${BODY:0:300})"
